...
class ContextOptimizer:
    ...
    Implements compression, chunking, and retrieval strategies to
    optimize token usage while preserving semantic meaning.
    """

    def __init__(
        self, max_context_length: int = 128000, target_compression_ratio: float = 0.5
    ):
    ...
    def optimize_context(
        self, content: str, strategy: str = "semantic", **kwargs
    ) -> CompressionResult:
        ...
        logger.info(
            f"Optimizing context with strategy: {strategy}, Input length: {len(content)}"
        )
    ...
    def _semantic_compression(self, content: str, **kwargs) -> str:
        """Compresses content by removing redundant words, stop words, and
        formatting while preserving key semantic meaning.
    ...
    def _hierarchical_compression(self, content: str, **kwargs) -> str:
        """Compresses by retaining headers and the first/last sentences of
        paragraphs.
    ...
    def _sliding_window(
        self, content: str, window_size: int = 2000, overlap: int = 200, **kwargs
    ) -> str:
        """Retains the most recent context (end of string) and a summary of the
        beginning."""
        ...
        summary_marker = (
            f"[...Previous {len(content) - window_size} characters summarized...]\n"
        )

        return summary_marker + recent_context

    def _selective_retention(
        self, content: str, keywords: List[str] = None, **kwargs
    ) -> str:
