...
Adds missing required fields to prompt markdown files based on the unified schema.
"""
...
DEFAULTS = {
    ...
    'intro': lambda title, desc: desc if desc else f"A prompt for {title.lower().replace('prompt', '').strip()}.",
    'type': 'how_to',
...
def format_frontmatter(fm: Dict[str, Any]) -> str:
    ...
    # Define field order
    order = [
        'title', 'shortTitle', 'intro', 'type', 'difficulty',
        'audience', 'platforms', 'topics', 'technique',
        'author', 'version', 'date',
        'governance_tags', 'dataClassification', 'reviewStatus'
    ]
...
def main():
    parser = argparse.ArgumentParser(description='Normalize frontmatter in prompt files')
