...
class LLMClient:
    ...
    def _call_azure_foundry(model_name: str, prompt: str, system_instruction: Optional[str],
                            temperature: float = 0.7, max_tokens: int = 4096) -> str:
        """
        Call Azure OpenAI API using REST endpoint.
        
        Model name format: azure-foundry:<endpoint_index>
        Examples:
          - azure-foundry:0 -> Uses AZURE_OPENAI_ENDPOINT_0
          - azure-foundry:1 -> Uses AZURE_OPENAI_ENDPOINT_1
          - azure-foundry:2 -> Uses AZURE_OPENAI_ENDPOINT_2
        
        Environment variables (from .env):
          - AZURE_OPENAI_API_KEY_N: API key for endpoint N
          - AZURE_OPENAI_ENDPOINT_N: Endpoint URL for endpoint N
        """
        # Parse endpoint index and optional deployment from model name
        # Format: azure-foundry:<endpoint_idx> or azure-foundry:<endpoint_idx>:<deployment>
        model_part = model_name.split(":", 1)[1] if ":" in model_name else "0"
        
        # Check if deployment is specified
        if ":" in model_part:
            endpoint_str, deployment = model_part.split(":", 1)
        else:
            endpoint_str = model_part
            deployment = None
        
        # Parse endpoint index, default to 0
        try:
            endpoint_idx = int(endpoint_str)
        except ValueError:
            endpoint_idx = 0
        
        # Get API key and endpoint for this index
        api_key = os.getenv(f"AZURE_OPENAI_API_KEY_{endpoint_idx}")
        endpoint = os.getenv(f"AZURE_OPENAI_ENDPOINT_{endpoint_idx}")
        
        if not api_key:
            raise ValueError(f"AZURE_OPENAI_API_KEY_{endpoint_idx} environment variable not set")
        if not endpoint:
            raise ValueError(f"AZURE_OPENAI_ENDPOINT_{endpoint_idx} environment variable not set")
        
        # Default deployment names per endpoint
        # afaifoundry deployments: gpt-oss-120b, grok-4-fast-reasoning, mistral-medium-2505
        if deployment is None:
            default_deployments = {
                0: "mistral-medium-2505",  # Fast, good for evaluation
            }
            deployment = default_deployments.get(endpoint_idx, "mistral-medium-2505")
        
        # Build the full URL with deployment name
        # Azure OpenAI format: {endpoint}/openai/deployments/{deployment}/chat/completions?api-version=X
        base_url = endpoint.rstrip("/")
        full_url = f"{base_url}/openai/deployments/{deployment}/chat/completions?api-version=2024-02-15-preview"
        ...
        try:
            ...
            raise RuntimeError(f"Azure OpenAI connection error: {e}")
