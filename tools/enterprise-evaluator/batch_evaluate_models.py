...
def evaluate_with_model(model_key: str, prompt_files: List[Path], output_dir: Path) -> Dict[str, Any]:
    """
    Evaluate all prompts with a single model.
    
    Supports both local models (phi4, mistral) and remote models (gh:gpt-4o, azure-foundry:phi4).
    ...
    for i, prompt_file in enumerate(prompt_files, 1):
        ...
        try:
            ...
            print(f"Score: {result['final_score']:.1f} ({result['performance_level']}) [{elapsed:.1f}s]")
...
def main():
    parser = argparse.ArgumentParser(description="Batch evaluate prompts across multiple local models")
    ...
    prompt_files = [
        f for f in sorted(prompts_dir.glob("*.md"))
    ...
    for s in all_summaries:
        if "error" not in s or s.get("successful", 0) > 0:
            print(f"{s['model']:<15} {s.get('avg_score', 0):>10.1f} {s.get('min_score', 0):>8.1f} {s.get('max_score', 0):>8.1f} {s.get('successful', 0):>7}/{s.get('total_prompts', 0)}")
        else:
