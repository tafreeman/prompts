...
try:
    ...
    for parent in [Path(__file__).parent, Path(__file__).parent.parent, Path(__file__).parent.parent.parent]:
        env_file = parent / ".env"
        if env_file.exists():
...
class EnterpriseEvaluator:
    def __init__(self, model_name: str = "local:phi4mini", temperature: float = 0.1, max_tokens: int = 1000):
    ...
    def _llm_judge_score(self, prompt_content: str, dimension: Dimension) -> EvaluationResult:
        """
...
"""
        ...
        try:
            ...
            if score is None:
                score_match = re.search(r'["\']?score["\']?\s*[:\s]\s*(\d+(?:\.\d+)?)', response, re.IGNORECASE)
                if score_match:
                    score = float(score_match.group(1))
                    # Try to get reasoning too
                    reason_match = re.search(r'["\']?reasoning["\']?\s*[:\s]\s*["\']([^"\']+)["\']', response, re.IGNORECASE)
                    if reason_match:
            ...
            reasoning = f"Error interpreting judge response: {str(e)}. Raw: {response[:100]}"
    ...
    def _eval_performance_reliability(self, content: str) -> EvaluationResult:
        ...
        # Run 1
        r1 = self.client.generate_text(self.model_name, content, temperature=self.temperature, max_tokens=100)
        # Run 2
        r2 = self.client.generate_text(self.model_name, content, temperature=self.temperature, max_tokens=100)
        # Run 3
        r3 = self.client.generate_text(self.model_name, content, temperature=self.temperature, max_tokens=100)
