...
def run_gh_models_eval(eval_file: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        print(
            "Error: 'gh' command not found. Install GitHub CLI and gh-models extension."
        )
        return None
...
def print_results(scores: List[Dict[str, Any]], eval_name: str):
    ...
    for i, s in enumerate(sorted_scores, 1):
        ...
        if s.get("scores"):
            ...
            advanced_parts = [
                f"{k}: {v}" for k, v in s["scores"].items() if k in advanced
            ]
    ...
    overall_scores = [
        s.get("overall_score", 0) for s in scores if s.get("overall_score")
    ]
...
def generate_markdown_report(
    all_results: Dict[str, List[Dict[str, Any]]], output_path: str
):
    ...
    if all_scores:
        overall_scores = [
            s.get("overall_score", 0) for s in all_scores if s.get("overall_score")
        ]
        ...
        if criterion_scores:
            ...
            for crit, vals in sorted(criterion_scores.items()):
                avg_crit = sum(vals) / len(vals)
                status = (
                    "✅ Pass" if avg_crit >= MIN_CRITERION_SCORE else "⚠️ Needs Work"
                )
    ...
    if all_scores:
        ...
        top = sorted(all_scores, key=lambda x: x.get("overall_score", 0), reverse=True)[
            :5
        ]
        for i, s in enumerate(top, 1):
            passed = "✅" if s.get("pass") else "❌"
            lines.append(
                f"{i}. **{s.get('title', 'Unknown')}** - {s.get('overall_score', 0)}/10 ({s.get('grade', 'N/A')}) {passed}"
            )
    ...
    needs_work = [
        s
        for s in all_scores
        if s.get("pass") is True and s.get("overall_score", 0) < 8.0
    ]
    if needs_work:
        lines.extend(
            [
...
def main():
    ...
    parser.add_argument(
        "eval_files", nargs="+", help="Evaluation file(s) to run (.prompt.yml)"
    )
    parser.add_argument("--report", "-r", help="Generate markdown report to this file")
    parser.add_argument(
        "--json-output", "-j", help="Save raw JSON results to this file"
    )
