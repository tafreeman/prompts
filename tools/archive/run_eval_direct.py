    ...
    while True:
        try:
            ...
            if 0 <= idx < len(models_list):
                ...
                if in_use:
                    confirm = input(f"\n⚠️  {name} appears to be in use. Continue anyway? (y/n): ").strip().lower()
                    if confirm != 'y':
                        continue
...
def main():
    ...
    prompts = [f for f in sorted(folder.glob("*.md")) 
               if f.name not in ["README.md", "index.md"]]
    
    print(f"\n{'='*60}")
    print(f"DIRECT EVALUATION (evaluate_prompt)")
    ...
    for i, p in enumerate(prompts, 1):
        ...
        try:
            ...
            if scores_detail:
                criteria_line = " | ".join(f"{k[:4]}:{v}" for k, v in scores_detail.items() if isinstance(v, (int, float)))
    ...
    print(f"\n{'='*60}")
    print(f"SUMMARY - Direct Evaluation")
    print(f"{'='*60}")
    print(f"Model: {model_name}")
