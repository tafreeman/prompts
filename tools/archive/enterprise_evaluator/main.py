...
def main():
    ...
    parser.add_argument(
        "target", nargs="?", help="Path to prompt file (.md) or directory to evaluate"
    )
    ...
    parser.add_argument(
        "--check", action="store_true", help="Check if configured model is available"
    )
    ...
    if args.check:
        ...
        try:
            ...
            test_response = LLMClient.generate_text(
                args.model, "Test connection", max_tokens=10
            )
        ...
        while True:
            try:
                ...
                response = LLMClient.generate_text(
                    args.model, user_input, temperature=args.temperature or 0.7
                )
                print("\n" + response)
