...
Uses Phi-3.5 Mini model. Saves results after EACH file.
"""
...
BATCH_SIZE = 3
OUTPUT_FILE = Path(__file__).parent.parent / "eval_geval_results_2.json"


def save_results(results, model_name="phi-3.5-mini"):
...
def main():
    ...
    print(f"\n{'='*60}")
    print("G-EVAL EVALUATION #2 (Batch Mode)")
    print("Model: Phi-3.5 Mini")
    ...
    model = LocalModel(
        model_path=str(
            Path.home()
            / ".cache/aigallery/microsoft--Phi-3.5-mini-instruct-onnx/main/cpu_and_mobile/cpu-int4-awq-block-128-acc-level-4"
        ),
        verbose=False,
    )
    create_model_lock("Phi-3.5 Mini")  # Mark model as in use
    print("✓ Model loaded\n")
    ...
    for i in range(0, len(prompts), BATCH_SIZE):
        ...
        for j, p in enumerate(batch, 1):
            ...
            try:
                ...
                if scores_detail:
                    criteria_line = " | ".join(
                        f"{k[:4]}:{v:.1f}" for k, v in scores_detail.items()
                    )
    ...
    print(f"\n{'='*60}")
    print("FINAL SUMMARY - G-Eval (Phi-3.5 Mini)")
    print(f"{'='*60}")
