...
def update_frontmatter_score(filepath: Path, new_score: float, dry_run: bool = False) -> bool:
    ...
    try:
        ...
        if dry_run:
            print(f"  [DRY RUN] Would update {filepath.name}: effectivenessScore = {new_score:.1f}")
            return True
...
def filter_pending_prompts(prompts: List[PromptInfo], skip_scored: bool = False) -> List[PromptInfo]:
...
def run_dual_eval(filepath: Path, runs: int = 2) -> EvaluationResult:
    ...
    result = EvaluationResult(
        prompt=PromptInfo(file_path=filepath)
    )
    ...
    try:
        ...
        score_match = re.search(r'Consensus Score[:\s]+(\d+\.?\d*)/10', output, re.IGNORECASE)
        if score_match:
        ...
        if analyzer_path.exists():
            ...
            if proc.returncode == 0 and proc.stdout:
                try:
                    ...
                    result.passed = result.new_score >= PASS_THRESHOLD if result.new_score else False
...
def generate_batch_report(report: BatchReport, output_path: Optional[Path] = None) -> str:
    ...
    if report.results:
        ...
        for result in sorted(report.results, key=lambda r: r.new_score or 0, reverse=True):
            status = "✅" if result.passed else "❌"
            if result.error:
                status = "⚠️"
            
            score_str = f"{result.new_score:.1f}" if result.new_score is not None else "N/A"
    ...
    low_scores = [r for r in report.results if r.new_score is not None and r.new_score < PASS_THRESHOLD]
    if low_scores:
        lines.extend([
            "## Prompts Needing Attention",
            "",
        ...
        for result in sorted(low_scores, key=lambda r: r.new_score or 0):
            lines.append(f"- **{result.prompt.file_path.name}**: {result.new_score:.1f}/10")
        lines.extend([""])
...
def main():
    ...
    parser.add_argument(
        "--folder",
        type=str,
        help="Specific folder to evaluate (relative to repo root)"
    )
    parser.add_argument(
        "--limit",
        type=int,
    ...
    # Discover prompts
    print(f"🔍 Scanning for prompts in: {base_dir / (args.folder or DEFAULT_PROMPTS_DIR)}")
    ...
    for i, prompt in enumerate(prompts_to_evaluate, 1):
        ...
        if args.simple:
            ...
            if result.error and "not found" not in result.error.lower():
                print(f"    ⚠️  Dual eval failed, trying simple evaluation...")
                result = run_simple_evaluation(prompt.file_path)
    ...
    if output_path is None and not args.dry_run:
        output_path = base_dir / DEFAULT_OUTPUT_DIR / f"batch_eval_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
