...
def aggregate_results(results: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    summary = {
        ...
        "successful_evals": sum(
            1 for r in results if r.get("overall", r.get("overall_score"))
        ),
    }
...
def main():
    parser = argparse.ArgumentParser(
        description="Batch evaluate prompts with smart model selection"
    )
    parser.add_argument("folder", help="Folder containing .prompt.yml files")
    parser.add_argument(
        "--output", "-o", default="batch_eval_results.json", help="Output file"
    )
    parser.add_argument("--gh-only", action="store_true", help="Only use GitHub models")
    parser.add_argument(
        "--local-only", action="store_true", help="Only use local models"
    )
    parser.add_argument(
        "--models",
        nargs="+",
        help="Specific GH models to use (overrides smart selection)",
    )
    parser.add_argument(
        "--allow-premium",
        action="store_true",
        help="Allow premium models for complex prompts",
    )
    parser.add_argument(
        "--show-complexity", action="store_true", help="Show complexity analysis only"
    )
