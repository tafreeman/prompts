...
def scan_prompts(
    folder: Path, limit: int = None, verbose: bool = True
) -> Dict[str, Any]:
    ...
    print(f"\n{'='*70}")
    print("DUAL EVALUATION SCAN")
    print(f"{'='*70}")
    print(f"Folder: {folder}")
    print(f"Prompts found: {len(prompts)}")
    print("Method 1: evaluate_prompt() - Direct scoring")
    print("Method 2: evaluate_prompt_geval() - G-Eval Chain-of-Thought")
    print(f"{'='*70}\n")
    ...
    for i, prompt_path in enumerate(prompts, 1):
        relative_path = (
            prompt_path.relative_to(folder.parent)
            if folder.parent in prompt_path.parents
        ...
        try:
            ...
            if verbose:
                print("  → Running evaluate_prompt()...", end=" ", flush=True)
            ...
            if verbose:
                print("  → Running evaluate_prompt_geval()...", end=" ", flush=True)
            ...
            agreement = (
                "high"
                if score_diff <= 0.5
                else ("low" if score_diff > 1.0 else "medium")
            )
            ...
            prompt_result = {
                ...
                "geval_details": {
                    c: r.get("score", 0)
                    for c, r in geval_result.get("criteria_results", {}).items()
                },
            }
            ...
            agreement_icon = (
                "✓" if agreement == "high" else ("⚠" if agreement == "medium" else "✗")
            )
            print(
                f"  → Combined: {combined_score} {stars} [{agreement_icon} {agreement} agreement]\n"
            )

        except Exception as e:
            print(f"  ✗ Error: {e}\n")
            results["prompts"].append({"file": str(relative_path), "error": str(e)})

    # Calculate summary stats
    if direct_scores:
        results["summary"]["direct_avg"] = round(
            sum(direct_scores) / len(direct_scores), 2
        )
    if geval_scores:
        results["summary"]["geval_avg"] = round(
            sum(geval_scores) / len(geval_scores), 2
        )

    all_combined = [
        p.get("combined_score", 0)
        for p in results["prompts"]
        if p.get("combined_score", 0) > 0
    ]
    if all_combined:
        results["summary"]["combined_avg"] = round(
            sum(all_combined) / len(all_combined), 2
        )
...
def main():
    ...
    parser.add_argument(
        "folder", nargs="?", default="prompts", help="Folder to scan (default: prompts)"
    )
    parser.add_argument(
        "--limit", "-l", type=int, help="Limit number of prompts to scan"
    )
