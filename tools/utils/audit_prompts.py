#!/usr/bin/env python3
"""Prompt Library Auditor Scans the prompt library (prompts/ folder only), runs
validation on all prompt files, and generates a migration report.

Excludes agent files, instruction files, and other non-prompt markdown
files.
"""
...
def audit_library(root_dir, output_file="audit_report.csv"):
    ...
    print(
        "Excluding: agent files, instruction files, index.md, README.md, docs/, agents/, templates/"
    )
            ...
            results.append(
                {
                    ...
                    "error_count": sum(
                        1 for issue in report.issues if issue.level.value == "error"
                    ),
                    "warning_count": sum(
                        1 for issue in report.issues if issue.level.value == "warning"
                    ),
                }
            )
            if (i + 1) % 10 == 0:
