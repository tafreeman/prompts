...
class AccuracyBenchmark:
    ...
    async def _call_llm_with_retry(self, prompt: str, test_input: str) -> LLMResponse:
        ...
        for attempt in range(MAX_RETRIES):
            ...
            if response.error and ("rate" in response.error.lower() or "429" in response.error):
    ...
    def _evaluate_response_quality(self, response: str, expected: Optional[str]) -> Tuple[bool, float]:
        ...
        not_error = not any(err in response.lower() for err in ['error', 'cannot', 'unable', 'sorry'])
    ...
    def evaluate(self, prompt: str, test_cases: List[TestCase]) -> BenchmarkResult:
        ...
        return loop.run_until_complete(
            self.evaluate_async(prompt, test_cases)
        )
    
    async def evaluate_async(self, prompt: str, test_cases: List[TestCase]) -> BenchmarkResult:
...
class LatencyBenchmark:
    ...
    async def _measure_real_latency(self, prompt: str, test_input: str) -> Tuple[float, bool]:
    ...
    async def evaluate_async(self, prompt: str, test_cases: List[TestCase]) -> BenchmarkResult:
        ...
        if self.use_real_llm and LLM_CLIENT_AVAILABLE:
            ...
            for i, test_case in enumerate(sample_cases):
                ...
                latency, success = await self._measure_real_latency(prompt, test_case.input_data)
                if success:
...
class CostBenchmark:
    ...
    def record_actual_usage(self, input_tokens: int, output_tokens: int):
        ...
        self._actual_tokens['input'] = self._actual_tokens.get('input', 0) + input_tokens
        self._actual_tokens['output'] = self._actual_tokens.get('output', 0) + output_tokens
...
class PromptPerformanceEvaluator:
    ...
    def evaluate_prompt(self, prompt: str, test_cases: List[TestCase] = None) -> PerformanceReport:
        ...
        try:
            ...
            asyncio.set_event_loop(loop)
        return loop.run_until_complete(
            self.evaluate_prompt_async(prompt, test_cases)
        )
    
    async def evaluate_prompt_async(self, prompt: str, test_cases: List[TestCase] = None) -> PerformanceReport:
    ...
    async def evaluate_prompt_file_async(self, prompt_file: str, test_cases: List[TestCase] = None) -> PerformanceReport:
...
def main():
    ...
    parser.add_argument("--use-real-llm", action="store_true", help="Use real LLM API calls instead of simulated values")
    parser.add_argument("--model", type=str, default=DEFAULT_MODEL, help=f"Model to use for LLM calls (default: {DEFAULT_MODEL})")
    ...
    if args.use_real_llm and not LLM_CLIENT_AVAILABLE:
        ...
        print("   Make sure tools/llm_client.py exists and required packages are installed.")
    ...
    if args.json:
        ...
        if report.effectivenessScore < args.min_score:
            print(f"❌ Performance below threshold (score {report.effectivenessScore:.1f} < {args.min_score})")
            exit(1)
        else:
