#!/usr/bin/env python3
"""Prompt Library Improvement Script Uses evaluation rubrics and improvement
prompts to systematically upgrade prompt quality.
...
def extract_frontmatter(content: str) -> Optional[Dict[str, Any]]:
    """Extract YAML frontmatter from markdown content (minimal: name,
    description)."""
...
def assess_prompt(file_path: Path) -> Optional[PromptAssessment]:
    ...
    title = (
        registry_entry.get("title", file_path.stem.replace("-", " ").title())
        if registry_entry
        else frontmatter.get("name", file_path.stem.replace("-", " ").title())
    )
    category = (
        registry_entry.get("categories", [file_path.parent.name])[0]
        if registry_entry and "categories" in registry_entry
    ...
    if has_description:
        ...
        assessment.issues.append(
            IssueItem(
                ...
                "Add ## Description with 2-3 sentences explaining the prompt",
                "Low",
                3,
            )
        )
    ...
    if has_prompt_section:
        ...
        assessment.issues.append(
            IssueItem(
                ...
                "Add ## Prompt with the actual prompt template",
                "Low",
                5,
            )
        )
    ...
    if has_variables:
        ...
        assessment.issues.append(
            IssueItem(
                ...
                "Add ## Variables documenting all placeholders",
                "Low",
                3,
            )
        )
    ...
    if has_example:
        ...
        assessment.issues.append(
            IssueItem(
                ...
                "Add ## Example Usage with realistic input/output",
                "Medium",
                4,
            )
        )
    ...
    if has_tips:
        ...
        assessment.issues.append(
            IssueItem(
                ...
                "Add ## Tips with 3-5 actionable recommendations",
                "Low",
                2,
            )
        )
    ...
    if has_example:
        ...
        if example_match:
            ...
            if example_lines >= 100:
                ...
                assessment.issues.append(
                    IssueItem(
                        "P1",
                        "example",
                        f"Example too short ({example_lines} lines)",
                        "Expand example with detailed realistic scenario (aim for 50+ lines)",
                        "Medium",
                        5,
                    )
                )
            ...
            if "**Input**" in example_content or "Input:" in example_content:
                ...
                assessment.issues.append(
                    IssueItem(
                        "P1",
                        "example",
                        "Example missing Input section",
                        "Add **Input:** showing filled-in variables",
                        "Low",
                        3,
                    )
                )
            ...
            if "**Output**" in example_content or "Output:" in example_content:
                ...
                assessment.issues.append(
                    IssueItem(
                        "P1",
                        "example",
                        "Example missing Output section",
                        "Add **Output:** showing expected AI response",
                        "Low",
                        3,
                    )
                )
            ...
            if re.search(
                r"\$[\d,]+|\d+%|\d+\s*(hours?|days?|minutes?)", example_content
            ):
                example_quality += 5

            if (
                "placeholder" in example_content.lower()
                or "[replace" in example_content.lower()
            ):
                example_quality -= 5
                assessment.issues.append(
                    IssueItem(
                        "P0",
                        "example",
                        "Example contains placeholder text",
                        "Replace all placeholders with realistic values",
                        "Low",
                        5,
                    )
                )
        ...
        assessment.issues.append(
            IssueItem(
                ...
                "example",
                "No example section",
                "Add complete example with realistic Input and Output",
                "Medium",
                10,
            )
        )
            ...
            if tips_count >= 5:
                ...
                assessment.issues.append(
                    IssueItem(
                        "P2",
                        "tips",
                        f"Only {tips_count} tips provided",
                        "Add 3-5 actionable, specific tips",
                        "Low",
                        2,
                    )
                )
            ...
            if not any(p in tips_content.lower() for p in generic_phrases):
                ...
                assessment.issues.append(
                    IssueItem(
                        "P2",
                        "tips",
                        "Tips contain generic phrases",
                        "Replace generic tips with specific, actionable recommendations",
                        "Low",
                        2,
                    )
                )
            ...
            if "example" in vars_content.lower() or "e.g." in vars_content.lower():
                ...
                assessment.issues.append(
                    IssueItem(
                        "P1",
                        "variables",
                        "Variables lack example values",
                        "Add realistic example values for each variable",
                        "Low",
                        3,
                    )
                )
        ...
        if use_case_count >= 5:
            ...
            assessment.issues.append(
                IssueItem(
                    "P2",
                    "use_cases",
                    f"Only {use_case_count} use cases",
                    "Add at least 5 specific use cases",
                    "Low",
                    2,
                )
            )
    ...
    if frontmatter.get("governance_tags"):
        ...
        assessment.issues.append(
            IssueItem(
                "P2",
                "governance",
                "Missing governance_tags",
                "Add governance_tags (e.g., PII-safe, requires-human-review)",
                "Low",
                2,
            )
        )
    ...
    assessment.quality_total = round(
        ...
        + assessment.example_quality
        + assessment.specificity
        + assessment.format_adherence
        + assessment.enterprise_quality,
        1,
    )
    ...
    if "## use case" in body_lower:
        reusability += 0.3
    if "## variation" in body_lower:
        reusability += 0.2
    ...
    if "## changelog" in body_lower:
        ...
        assessment.issues.append(
            IssueItem(
                ...
                "Remove Changelog (use git history instead)",
                "Low",
                2,
            )
        )
    ...
    assessment.effectiveness_rating = get_effectiveness_rating(
        assessment.effectiveness_total
    )
...
def generate_improvement_report(
    assessments: List[PromptAssessment], output_path: Optional[Path] = None
) -> str:
    ...
    if critical:
        ...
        for a in critical:
            ...
            lines.append(
                f"**Scores**: Quality {a.quality_total}/100, Effectiveness {a.effectiveness_total}/5.0"
            )
            lines.append("")
    ...
    if high:
        ...
        for a in high:
            ...
            lines.append(
                f"| `{Path(a.file_path).name}` | {a.quality_total:.0f} | {a.effectiveness_total:.1f} | {top_issue[:40]}... |"
            )
        lines.append("")
        lines.append("---")
        lines.append("")
    ...
    if medium:
        ...
        for a in medium[:20]:  # Top 20
            ...
            lines.append(
                f"| `{Path(a.file_path).name}` | {a.quality_total:.0f} | {a.effectiveness_total:.1f} | {issue_count} issues |"
            )
        lines.append("")
        lines.append("---")
        lines.append("")
    ...
    for a in critical[:5]:
        lines.append(
            f"- [ ] Fix `{Path(a.file_path).name}` (Quality: {a.quality_total:.0f})"
        )
    ...
    for a in high[:10]:
        lines.append(
            f"- [ ] Improve `{Path(a.file_path).name}` (Quality: {a.quality_total:.0f})"
        )
    ...
    for a in medium[:15]:
        lines.append(
            f"- [ ] Polish `{Path(a.file_path).name}` (Quality: {a.quality_total:.0f})"
        )
...
def generate_improvement_prompts_file(
    assessments: List[PromptAssessment], output_path: Path, count: int = 10
):
...
Examples:
  ...
  python tools/improve_prompts.py --all --generate-tasks
    ...
    parser.add_argument(
        "--folder", type=str, help="Assess prompts in a specific folder"
    )
    parser.add_argument("--worst", type=int, default=0, help="Show N worst prompts")
    parser.add_argument(
        "--generate-tasks", action="store_true", help="Generate improvement task file"
    )
    parser.add_argument(
        "--generate-prompts",
        action="store_true",
        help="Generate improvement prompts file",
    )
    parser.add_argument(
        "--output",
        "-o",
        type=str,
        default="docs/IMPROVEMENT_PLAN.md",
