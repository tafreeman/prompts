...
HEADING_RE = re.compile(r"^##\s+(Description|Prompt|Variables|Example)\b", re.IGNORECASE | re.MULTILINE)
...
def ensure_frontmatter(text: str, filename: str):
    ...
    if m:
        ...
        keys = {k.strip(): v.strip() for k, v in (line.split(':', 1) for line in fm.splitlines() if ':' in line)}
        changed = False
        if 'name' not in keys:
            fm = fm + f"\nname: {filename}\n"
            changed = True
