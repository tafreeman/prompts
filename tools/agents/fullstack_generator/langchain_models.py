...
class ModelInfo:
    ...
    def from_model_string(cls, model: str) -> "ModelInfo":
        ...
        if model.startswith("local:"):
            ...
            return cls(model_id=model, provider="ollama", is_local=True, is_free=True)
            ...
            return cls(model_id=model, provider="aitk", is_local=True, is_free=True)
        else:
            return cls(
                model_id=model, provider="unknown", is_local=False, is_free=False
            )
        ...
        try:
            ...
            if response.startswith("Error:") or response.startswith("gh models error:"):
                if self.fallback_model:
                    print(
                        f"  [Fallback] {self.model} failed, trying {self.fallback_model}"
                    )
                    response = LLMClient.generate_text(
            ...
            if self.fallback_model:
                print(
                    f"  [Fallback] {self.model} error: {e}, trying {self.fallback_model}"
                )
