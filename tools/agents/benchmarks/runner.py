...
from tools.agents.benchmarks.datasets import BENCHMARK_DEFINITIONS, BenchmarkType
...
from tools.agents.benchmarks.loader import load_benchmark, clear_cache, BenchmarkTask
...
def select_model() -> Optional[str]:
    ...
    provider_display = {
        "github_models": ("🌐 GitHub Models", ["gh:openai/gpt-4o-mini", "gh:openai/gpt-4o", "gh:microsoft/phi-4"]),
        "local_onnx": ("💻 Local ONNX", ["local:phi4", "local:phi3.5", "local:mistral"]),
        "ollama": ("🦙 Ollama", ["ollama:deepseek-r1:8b", "ollama:qwen2.5-coder:14b", "ollama:phi4-reasoning:latest"]),
        "ai_toolkit": ("🧰 AI Toolkit", ["aitk:phi-4-mini-instruct", "aitk:qwen2.5-coder-7b-instruct"]),
    }
...
def configure_options(config: BenchmarkConfig) -> BenchmarkConfig:
    ...
    # Task limit
    limit_str = prompt_input("Max tasks to run (empty for all)", str(config.limit or ""))
    ...
    # Timeout
    timeout_str = prompt_input("Timeout per task (seconds)", str(config.timeout_seconds))
...
def evaluate_task_output_llm(
    ...
    if verbose:
        ...
        print(f"  Score: {eval_result.overall_score:.1f}/10.0 (Grade: {eval_result.grade})")
...
def evaluate_task_output_legacy(task: BenchmarkTask, output: str, verbose: bool = False, output_dir: Optional[Path] = None) -> Optional[Dict[str, Any]]:
    """
...
def print_mismatch_analysis(eval_result: Dict[str, Any], gold_data: Dict[str, Any], output: str) -> None:
...
def save_evaluation_report_legacy(task_id: str, eval_result: Dict[str, Any], gold_data: Dict[str, Any], output: str, output_dir: Path) -> None:
    """Save detailed evaluation report to file (legacy pattern-matching format)."""
...
def evaluate_agent_output(
    ...
    try:
        ...
        return {
            ...
            "dimension_scores": {k: v.score for k, v in eval_result.dimension_scores.items()},
...
def get_agent_expectations(agent_type: str) -> Dict[str, str]:
    ...
    return expectations.get(agent_type, {
...
def extract_workflow_data(result, evaluate_phases: bool = False, model: str = None, 
                          benchmark_id: str = None, original_prompt: str = None,
                          verbose: bool = False) -> Dict[str, Any]:
    """
    Extract full workflow data from an OrchestratorResult.
    
    Captures all phases, agent outputs, and metadata for comprehensive logging.
    Optionally evaluates each agent's output.
    """
    workflow_data = {
    ...
    if workflow_data["phase_evaluations"]:
        scores = [e.get("score", 0) for e in workflow_data["phase_evaluations"].values()]
        workflow_data["phase_summary"] = {
            "total_agents": len(scores),
            "average_score": sum(scores) / len(scores) if scores else 0,
            "min_score": min(scores) if scores else 0,
            "max_score": max(scores) if scores else 0,
            "scores_by_agent": {k: v.get("score", 0) for k, v in workflow_data["phase_evaluations"].items()},
        }
    
    return workflow_data


def save_workflow_phases_md(workflow_data: Dict[str, Any], task_id: str, output_dir: Path) -> None:
    """
    ...
    if phase_summary:
        ...
        lines.append(f"**Score Range:** {phase_summary.get('min_score', 0):.1f} - {phase_summary.get('max_score', 0):.1f}")
        lines.append("")
...
def run_benchmark(config: BenchmarkConfig) -> Dict[str, Any]:
    ...
    output_dir = Path(__file__).parents[3] / "results" / "benchmark_runs" / f"{config.benchmark_id}_{timestamp}"
    ...
    for i, task in enumerate(tasks, 1):
        ...
        try:
            ...
            if orchestrator:
                ...
                print(f"  Evaluating individual agent outputs...")
