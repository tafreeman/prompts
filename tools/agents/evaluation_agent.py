...
class AgentConfig:
    ...
    # Paths - go up 2 levels from tools/ to repo root
...
def setup_logging(verbose: bool = False) -> logging.Logger:
    ...
    log_file = (
        AgentConfig.ROOT_DIR
        / "logs"
        / f"eval_agent_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
    )
...
def save_checkpoint(state: AgentState):
    ...
    AgentConfig.CHECKPOINT_FILE.write_text(
        json.dumps(checkpoint_data, indent=2, default=str), encoding="utf-8"
    )
    ...
    for attempt in range(AgentConfig.MAX_RETRIES):
        try:
            ...
            if result.returncode == 0:
                ...
                logger.warning(
                    f"Command failed (attempt {attempt + 1}): {result.stderr[:200]}"
                )
...
def check_prerequisites(logger: logging.Logger) -> bool:
    ...
    if not success:
        logger.error(
            "gh-models extension not found. Install with: gh extension install github/gh-models"
        )
        return False
    ...
    # Check required scripts (in tools/)
    required_scripts = [
        AgentConfig.ROOT_DIR / "tools" / "generate_eval_files.py",
        AgentConfig.ROOT_DIR / "tools" / "run_gh_eval.py",
...
def count_prompts_in_category(category: str) -> int:
    ...
    count = sum(1 for f in category_dir.glob("*.md") if f.name.lower() not in excluded)
    return count


def generate_eval_files(
    category: str, logger: logging.Logger, dry_run: bool = False
) -> TaskResult:
    ...
    cmd = [
        "python",
        str(AgentConfig.ROOT_DIR / "tools" / "generate_eval_files.py"),
    ...
    task.duration_seconds = (
        datetime.fromisoformat(task.end_time) - datetime.fromisoformat(task.start_time)
    ).total_seconds()
...
def run_evaluations(
    category: str, model: str, runs: int, logger: logging.Logger, dry_run: bool = False
) -> TaskResult:
    ...
    def run_single_eval(eval_file, run_num):
        ...
        success, stdout, stderr = run_command(
            cmd, logger, timeout=AgentConfig.EVAL_TIMEOUT_SECONDS
        )
    ...
    logger.info(
        f"   📊 Running {len(eval_tasks)} evaluations with {AgentConfig.MAX_PARALLEL_EVALS} parallel workers..."
    )

    # Run evaluations in parallel
    with ThreadPoolExecutor(max_workers=AgentConfig.MAX_PARALLEL_EVALS) as executor:
        futures = {
            executor.submit(run_single_eval, ef, rn): (ef, rn) for ef, rn in eval_tasks
        }
        ...
        for future in as_completed(futures):
            ...
            try:
                ...
                logger.debug(
                    f"   [{completed}/{len(eval_tasks)}] {eval_file.name} run {run_num}"
                )
            except Exception as e:
...
def parse_evaluation_results(category: str, logger: logging.Logger) -> CategoryResult:
            ...
            try:
                ...
                for run_result in data:
                    for test_result in run_result.get("testResults", []):
                        prompt_title = test_result.get("testCase", {}).get(
                            "promptTitle", "unknown"
                        )
...
def generate_improvement_plan(
    failing_prompts: List[Dict[str, Any]], logger: logging.Logger, dry_run: bool = False
) -> TaskResult:
    ...
    logger.info(
        f"📋 Generating improvement plan for {len(failing_prompts)} failing prompts..."
    )
    ...
    cmd = [
        "python",
        str(AgentConfig.ROOT_DIR / "tools" / "improve_prompts.py"),
        "--all",
        "--output",
        str(AgentConfig.REPORTS_DIR / "IMPROVEMENT_PLAN.md"),
    ]
...
def generate_final_report(
    state: AgentState, logger: logging.Logger, dry_run: bool = False
) -> TaskResult:
    ...
    cmd = [
        "python",
        str(AgentConfig.ROOT_DIR / "tools" / "evaluate_library.py"),
        "--all",
        "--output",
        str(report_path),
    ]
...
class EvaluationAgent:
    ...
    def __init__(
        self, logger: logging.Logger, dry_run: bool = False, resume: bool = False
    ):
        self.logger = logger
    ...
    def initialize(self) -> bool:
        ...
        if self.resume:
            ...
            if self.state:
                self.logger.info(
                    f"📂 Resuming from checkpoint (phase {self.state.current_phase})"
                )
                return True
            else:
                self.logger.warning("No checkpoint found, starting fresh")

        self.state = AgentState(started_at=datetime.now().isoformat())
        return True
    ...
    def run_phase(self, phase: int) -> bool:
        ...
        categories = [
            cat
            for cat, config in AgentConfig.CATEGORY_CONFIG.items()
    ...
    def evaluate_category(self, category: str) -> bool:
        ...
        task = run_evaluations(
            category, config["model"], config["runs"], self.logger, self.dry_run
        )
        self.state.task_history.append(task)
    ...
    def run_full_pipeline(self) -> bool:
        ...
        # Count total prompts
        total_expected = sum(
            count_prompts_in_category(cat) for cat in AgentConfig.CATEGORIES
        )
    ...
    def print_final_summary(self):
        ...
        self.logger.info("\n📁 Reports generated:")
        self.logger.info("   - docs/reports/EVALUATION_REPORT.md")
        self.logger.info("   - docs/reports/AGENT_EXECUTION_SUMMARY.md")
        if self.state.total_failed > 0:
            self.logger.info("   - docs/reports/IMPROVEMENT_PLAN.md")
...
Examples:
  ...
  python tools/evaluation_agent.py --full --verbose
    ...
    parser.add_argument(
        "--full", action="store_true", help="Run full evaluation pipeline"
    )
    parser.add_argument(
        "--phase", type=int, choices=[1, 2, 3, 4], help="Run specific phase (1-4)"
    )
    parser.add_argument(
    ...
    agent = EvaluationAgent(logger=logger, dry_run=args.dry_run, resume=args.resume)
