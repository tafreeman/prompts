#!/usr/bin/env python3
"""Test all model providers to ensure encoding and connectivity work
correctly."""
...
def _test_model(
    model_name: str, prompt: str = "Say 'Hello World' in exactly 2 words."
) -> dict:
    """Internal helper to test a single model.

    Returns a dict for reporting. This function is not a pytest test
    itself to avoid fixture resolution issues.
    """
    try:
        ...
        success = bool(
            ...
            and "error" not in response.lower()[:100]
        )
        return {
            "model": model_name,
