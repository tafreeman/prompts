...
def main(args: Optional[list] = None) -> int:
    ...
    eval_parser = subparsers.add_parser(
        "evaluate", 
        aliases=["eval", "e"],
        help="Evaluate prompts (get scores)"
    )
    eval_parser.add_argument(
        "path",
        type=Path,
        help="Prompt file or directory to evaluate"
    )
    eval_parser.add_argument(
        "--tier", "-t",
        type=int,
    ...
    eval_parser.add_argument(
        "--model", "-m",
        type=str,
        help="Specific model to use (overrides tier)"
    )
    eval_parser.add_argument(
        "--threshold",
        type=float,
    ...
    eval_parser.add_argument(
        "--output", "-o",
        type=Path,
        help="Output file (JSON or Markdown)"
    )
    eval_parser.add_argument(
        "--quiet", "-q",
        action="store_true",
    ...
    val_parser = subparsers.add_parser(
        "validate",
        aliases=["val", "v"],
        help="Validate prompt structure (no LLM)"
    )
    val_parser.add_argument(
        "path",
        type=Path,
        help="Prompt file or directory to validate"
    )
    val_parser.add_argument(
        "--strict",
        action="store_true",
    ...
    gen_parser = subparsers.add_parser(
        "generate",
        aliases=["gen", "g"],
        help="Generate text with an LLM"
    )
    ...
    models_parser.add_argument(
        "--probe",
        action="store_true",
        help="Test which models are actually available"
    )
...
def _cmd_evaluate(args) -> int:
    ...
    from .evaluate import evaluate, TIERS
...
def _cmd_validate(args) -> int:
    ...
    from .validate import validate, validate_batch, print_validation_summary
    ...
    if path.is_file():
        ...
        if result.is_valid:
            ...
            print(f"✗ {path}: {result.error_count} errors, {result.warning_count} warnings")
...
def _write_output(result, output_path: Path) -> None:
    ...
    if output_path.suffix == ".json":
        ...
        for r in result.results:
            ...
            lines.append(f"| {Path(r.file).name} | {r.score:.0f} | {r.grade} | {status} |")
