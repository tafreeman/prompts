...
class TestOrchestrator:
    ...
    async def run_suite(self, suite_path: str, **kwargs) -> dict:
        ...
        results = await self.runner.run_test_suite(suite_name, **kwargs)
...
async def main():
    ...
    parser = argparse.ArgumentParser(description="Run prompt testing framework")
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Dry run - don't execute tests"
    )
    ...
    try:
        # Prepare kwargs
        kwargs = {"parallel": args.parallel, "max_workers": args.max_workers}
