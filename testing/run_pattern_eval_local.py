...
def main():
    parser = argparse.ArgumentParser(
        description="Evaluate pattern conformance with local models"
    )
    ...
    # Load files
    print(f"\n{'='*60}")
    print(" Pattern Evaluation with Local Models")
    print(f"{'='*60}\n")
    ...
    if pattern_name is None:
        ...
        if pattern_name is None:
            print(
                "Error: Could not auto-detect pattern. Specify --pattern explicitly.",
                file=sys.stderr,
            )
    ...
    print(
        f"Overall Score: {result.overall_score:.2f}/5.0 ({result.overall_score * 20:.1f}/100)"
    )
    ...
    if result.failure_summary and result.failure_summary.failure_count > 0:
        ...
        for mode in result.failure_summary.failure_modes:
            count = sum(
                1 for f in result.failure_summary.failures if f.mode.value == mode
            )
