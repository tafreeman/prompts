...
class SafetyValidator(BaseValidator):
    ...
    def __init__(
        ...
        self.block_on_critical = (
            config.get("block_on_critical", True) if config else True
        )
    ...
    def get_safety_report(self) -> Dict[str, Any]:
        ...
        if any(v.category == "pii" for v in self.violations):
            report["recommendations"].append(
                "Implement PII redaction before storing or displaying"
            )

        if any(v.category == "security" for v in self.violations):
            report["recommendations"].append(
                "Review and remove all secrets and credentials"
            )

        if any(v.category == "harmful" for v in self.violations):
            report["recommendations"].append(
                "Review potentially harmful commands and add safeguards"
            )
...
class ContentModerationValidator(SafetyValidator):
    ...
    async def validate_content_policy(
        self, output: str, policy: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Validate against specific content policy."""

        results = {"compliant": True, "violations": [], "warnings": []}
        ...
        if "required_disclaimers" in policy:
            for disclaimer_type, disclaimer_text in policy[
                "required_disclaimers"
            ].items():
                if disclaimer_type in ["medical", "financial", "legal"]:
                    # Check if relevant content exists
                    pattern = (
                        self.content_rules[3].pattern
                        if disclaimer_type == "medical"
                        else (
                            self.content_rules[4].pattern
                            if disclaimer_type == "financial"
                            else r"(?i)\b(legal|law|attorney|court)\b"
                        )
                    )

                    if re.search(pattern, output) and disclaimer_text not in output:
                        results["warnings"].append(
                            f"Missing required {disclaimer_type} disclaimer"
                        )
