...
Prints outputs after each step to find where outputs become empty or
fail.
"""
...
DATASET_PATH = os.path.join(
    os.path.dirname(__file__), "../multiagent-workflows/ui/data/swebench.json"
)
...
async def run_step_by_step():
    ...
    while len(completed) < len(steps):
        ready_steps = [
            s_name
            for s_name in step_map
            if s_name not in completed
            and all(dep in completed for dep in step_deps.get(s_name, []))
        ]
        ...
        async def run_step(step_name):
            ...
            step_inputs = {
                k: artifacts[k] for k in step.required_inputs if k in artifacts
            }
            ...
            print(
                f"Output from {step.name}: {json.dumps(result.output, indent=2)[:800]}{'...' if len(json.dumps(result.output)) > 800 else ''}"
            )
        ...
        results = await asyncio.gather(
            *(run_step(s_name) for s_name in ready_steps), return_exceptions=True
        )
