...
def is_prompt_file(path: Path) -> bool:
    """Check if a markdown file is an actual prompt file (not agent,
    instruction, etc.).
...
def create_log_writer(
    log_file: Optional[str], prompt_data: Dict[str, Any]
...
"""
    ...
    temp_file = tempfile.NamedTemporaryFile(
        mode="w", suffix=".prompt.yml", delete=False, encoding="utf-8"
    )
...
def run_single_eval(eval_file: str, model: str, run_number: int) -> EvalResult:
...
def run_model_evaluations(
    ...
    """Run all evaluation runs for a single model.
        ...
        for run_num in range(1, runs + 1):
            ...
            if result.error:
                ...
                if fatal_reason:
                    ...
                    print(
                        f"    ⛔ Fatal error detected ({fatal_reason}). Skipping remaining runs for {model}."
                    )
                    break
                ...
                if consecutive_failures >= MAX_CONSECUTIVE_FAILURES:
                    print(
                        f"    ⚠️  Skipping remaining runs for {model} (too many failures)"
                    )
                    break
            else:
    ...
    scores = [
        ...
        if not r.error and r.overall_score > 0
    ]
    ...
    if scores:
        ...
        summary.pass_rate = sum(1 for r in summary.all_results if r.passed) / len(
            summary.all_results
        )
...
def run_parallel_evaluations(
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = {
            executor.submit(
                run_model_evaluations, prompt_data, model, runs_per_model, log_writer
            ): model
            for model in models
        }
...
def cross_validate(
    prompt_data: Dict[str, Any], model_summaries: Dict[str, ModelSummary]
    ...
    # Count total runs
    report.total_runs = sum(
        s.runs_completed + s.runs_failed for s in model_summaries.values()
    )
...
def generate_report(report: CrossValidationReport) -> str:
    ...
    if all_criteria:
        header = (
            "| Criterion | "
            + " | ".join(m.split("/")[-1][:12] for m in report.models_used)
            + " | Avg |"
        )
    ...
    for model, summary in report.model_summaries.items():
        ...
        for result in summary.all_results:
            if result.error:
                lines.append(
                    f"- **Run {result.run_number}**: ❌ Error - {result.error[:50]}"
                )
            else:
...
def print_summary(report: CrossValidationReport):
    ...
    print(
        f"   Cross-Validation: {'✅ Passed' if report.cross_validation_passed else '⚠️ High Variance'}"
    )
    ...
    for model, summary in report.model_summaries.items():
        ...
        status = "✅" if summary.avg_score >= PASS_THRESHOLD else "❌"
        print(
            f"   {status} {model}: {summary.avg_score:.1f}/10 (±{summary.std_dev:.2f})"
        )
...
def report_to_dict(report: CrossValidationReport) -> Dict[str, Any]:
    ...
    for model, summary in report.model_summaries.items():
        ...
        model_summaries_dict[model] = {
            ...
            "criterion_averages": {
                k: round(v, 2) for k, v in summary.criterion_averages.items()
            },
...
def generate_batch_markdown_report(reports: List[CrossValidationReport]) -> str:
    ...
    lines = [
        ...
        f"| **Failed** | {failed} ❌ |",
        (
            f"| **Pass Rate** | {passed/len(reports)*100:.1f}% |"
            if reports
            else "| **Pass Rate** | N/A |"
        ),
    ...
    for r in sorted(reports, key=lambda x: x.consensus_score, reverse=True):
        ...
        lines.append(
            f"| `{file_name}` | {r.consensus_score:.1f} | {r.final_grade} | {status} |"
        )
...
def print_batch_summary(reports: List[CrossValidationReport]):
    ...
    if failing:
        ...
        for r in failing:
            print(
                f"   - {Path(r.prompt_file).name}: {r.consensus_score:.1f}/10 ({r.final_grade})"
            )
...
def evaluate_single_file(
    ...
    model_summaries = run_parallel_evaluations(
        prompt_data, models, runs, max_workers, log_writer
    )
    ...
    if not quiet:
        ...
        print(
            f"   Result: {report.consensus_score:.1f}/10 ({report.final_grade}) {status}"
        )
...
Examples:
  ...
  python testing/evals/dual_eval.py "prompts/**/*.md" --format json
    ...
    parser.add_argument(
        "--log-file", help="Append per-run results to this markdown file in real-time"
    )
    parser.add_argument(
        "--max-workers",
        type=int,
    ...
    if args.changed_only:
        ...
        prompt_files = get_changed_files(
            base_ref=args.base_ref, paths=args.paths, include_all=args.include_all
        )
        if not prompt_files:
        ...
        prompt_files = discover_prompt_files(
            args.paths, recursive=not args.no_recursive, include_all=args.include_all
        )
        if not prompt_files:
    ...
    print(
        f"   Using {len(valid_models)} model(s): {', '.join(m.split('/')[-1] for m in valid_models)}"
    )
    ...
    for prompt_file in prompt_files:
        ...
        if report:
            ...
            errors.append(
                {"file": str(prompt_file), "error": "Failed to parse or evaluate"}
            )
    ...
    if args.output:
        ...
        if len(prompt_files) == 1:
            output_path = (
                Path("testing/evals/results") / f"{prompt_files[0].stem}_dual_eval{ext}"
            )
        else:
