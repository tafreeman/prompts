...
async def run_orchestration_test():
    ...
    try:
        ...
        if graph:
            ...
            print(
                "⚠️ LangGraph export returned None (LangGraph might not be installed)"
            )
    except Exception as e:
    ...
    try:
        checkpoint_dir = os.path.join(
            os.path.dirname(__file__), "_recovery/orch_test_checkpoints"
        )
        results = await workflow.execute(
            {"requirements": requirements}, checkpoint_dir=checkpoint_dir
        )
