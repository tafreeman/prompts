...
class CodeValidator(BaseValidator):
    ...
    async def validate(self, output: Any, expected: Optional[Any] = None) -> bool:
        ...
        if expected and isinstance(expected, dict):
            ...
            structure_valid = self._validate_structure(
                code, expected.get("structure", {})
            )

            return (
                syntax_valid and patterns_valid and contains_valid and structure_valid
            )
    ...
    async def _validate_with_compiler(self, code: str) -> bool:
        ...
        try:
            ...
            with tempfile.NamedTemporaryFile(
                mode="w", suffix=self.lang_config["extension"], delete=False
            ) as f:
                f.write(code)
                temp_file = f.name
    ...
    def _validate_structure(self, code: str, structure: Dict[str, Any]) -> bool:
        ...
        if self.language == "python":
            ...
            self.add_warning(
                f"Structure validation not implemented for {self.language}"
            )
            return True
    ...
    async def _run_test_case(
        ...
        if self.language == "python":
            ...
            raise NotImplementedError(
                f"Execution validation not implemented for {self.language}"
            )
...
expected_output = {json.dumps(test_case.get('output'))}
    ...
    # (AST parsing here is unnecessary for execution; remove to avoid
    # Python-version-dependent f-string escape syntax in generated code.)
