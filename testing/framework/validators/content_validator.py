...
class ContentValidator(BaseValidator):
    ...
    RECOMMENDED_SECTIONS = [
        'description', 'purpose', 'prompt', 'example', 'usage', 'tips'
    ]
    ...
    STRONG_VERBS = [
        'analyze', 'create', 'design', 'develop', 'evaluate',
        'generate', 'implement', 'review', 'transform', 'validate',
        'summarize', 'extract', 'identify', 'compare', 'explain'
    ]
    ...
    async def validate(self, output: Any, expected: Optional[Any] = None) -> bool:
        ...
        if placeholders:
            for placeholder in placeholders:
                self.add_error(f"Incomplete content: placeholder found - '{placeholder}'")
            is_valid = False
        ...
        if quality_score < self.min_quality_score:
            self.add_warning(f"Content quality score ({quality_score:.2f}) below threshold ({self.min_quality_score})")
            if self.strict_mode:
                is_valid = False
    ...
    def _validate_length(self, content: str) -> bool:
        ...
        if word_count < 50:
            self.add_warning(f"Content may be too short ({word_count} words). Consider adding more detail.")
        
        # Check maximum length
        if word_count > 3000:
            self.add_warning(f"Content may be too long ({word_count} words). Consider breaking into sections.")
        ...
        if prompt_match:
            ...
            if prompt_words < self.MIN_LENGTHS['prompt']:
                self.add_warning(f"Prompt section may be too short ({prompt_words} words)")
            elif prompt_words > self.MAX_LENGTHS['prompt']:
                self.add_warning(f"Prompt section may be too long ({prompt_words} words)")
    ...
    def _check_best_practices(self, content: str) -> None:
        ...
        format_patterns = ['output format', 'response format', 'return as', 'format:', 'respond in']
        ...
        constraint_patterns = ['do not', "don't", 'avoid', 'must', 'should', 'constraint', 'limit']
        has_constraints = any(p in content_lower for p in constraint_patterns)
        if not has_constraints:
            self.add_warning("Consider adding constraints or boundaries for the response")
    ...
    def get_content_analysis(self, content: str) -> Dict[str, Any]:
        ...
        has_format = any(p in content_lower for p in ['output format', 'response format'])
