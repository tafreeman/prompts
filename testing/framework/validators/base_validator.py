    ...
    def _deep_compare(self, actual: Any, expected: Any, path: str = "") -> bool:
        ...
        if isinstance(expected, dict) and isinstance(actual, dict):
            ...
            if len(expected) != len(actual):
                self.add_error(f"List length mismatch at {path}: expected {len(expected)}, got {len(actual)}")
                return False
            ...
            if actual != expected:
                self.add_error(f"Value mismatch at {path}: expected {expected}, got {actual}")
                return False
            return True
