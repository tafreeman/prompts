...
class FormatValidator(BaseValidator):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.required_fields = (
            set(config.get("required_fields", self.REQUIRED_FIELDS))
            if config
    ...
    def _validate_fields(self, frontmatter: Dict) -> bool:
        ...
        if "type" in frontmatter:
            ...
            if type_value not in self.VALID_TYPES:
                self.add_warning(
                    f"Unknown type '{type_value}'. Valid types: {', '.join(self.VALID_TYPES)}"
                )
        ...
        if "difficulty" in frontmatter:
            ...
            if diff_value not in self.VALID_DIFFICULTIES:
                self.add_warning(
                    f"Unknown difficulty '{diff_value}'. Valid values: {', '.join(self.VALID_DIFFICULTIES)}"
                )
            ...
            if isinstance(platforms, list):
                for platform in platforms:
                    if str(platform).lower() not in self.VALID_PLATFORMS:
                        self.add_warning(
                            f"Unknown platform '{platform}'. Valid platforms: {', '.join(self.VALID_PLATFORMS)}"
                        )
            ...
            if score != "pending":
                try:
                    ...
                    if score_float < 1.0 or score_float > 5.0:
                        self.add_warning(
                            f"effectivenessScore {score_float} is out of range (1.0-5.0)"
                        )
    ...
    def _validate_markdown(self, body: str) -> bool:
        ...
        for i, line in enumerate(lines, 1):
            ...
            if broken_links:
                self.add_warning(
                    f"Line {i}: Potentially broken markdown link (space between ] and ()"
                )
    ...
    def _validate_headings(self, body: str) -> bool:
        ...
        for hashes, text in headings:
            ...
            if level > prev_level + 1 and prev_level > 0:
                self.add_warning(
                    f"Heading hierarchy skip: '{text}' (h{level}) after h{prev_level}"
                )

            # Check for very deep headings
            if level > 4:
                self.add_warning(
                    f"Deep heading level (h{level}): '{text}'. Consider restructuring."
                )
    ...
    def _validate_code_blocks(self, body: str) -> bool:
        ...
        for code in inline_code:
            if len(code) > 100:
                self.add_warning(
                    f"Very long inline code ({len(code)} chars). Consider using a code block."
                )
