...
class SemanticValidator(BaseValidator):
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.min_coherence_score = (
            config.get("min_coherence_score", 0.6) if config else 0.6
        )
        self.require_clear_intent = (
            config.get("require_clear_intent", True) if config else True
        )
    ...
    async def validate(self, output: Any, expected: Optional[Any] = None) -> bool:
        ...
        if contradictions:
            for contra in contradictions:
                self.add_warning(
                    f"Potential contradiction detected: '{contra[0]}' vs '{contra[1]}'"
                )
        ...
        if coherence_score < self.min_coherence_score:
            self.add_warning(
                f"Low coherence score: {coherence_score:.2f} (minimum: {self.min_coherence_score})"
            )
        ...
        if expected and isinstance(expected, dict):
            ...
            if expected_intent and expected_intent not in detected_intents:
                self.add_error(
                    f"Expected intent '{expected_intent}' not detected in content"
                )
                is_valid = False
    ...
    def _validate_intent(self, content: str) -> Tuple[bool, List[str]]:
        ...
        if not detected_intents:
            self.add_warning(
                "No clear intent detected in prompt. Consider adding action verbs."
            )
            return False, []
        ...
        if len(detected_intents) > 3:
            self.add_warning(
                f"Multiple intents detected ({len(detected_intents)}). Consider focusing the prompt."
            )
    ...
    def _check_completeness(self, content: str, intents: List[str]) -> List[str]:
        ...
        if "analysis" in intents:
            ...
            if not any(elem in content_lower for elem in analysis_elements):
                issues.append(
                    "Analysis prompts should specify criteria or aspects to consider"
                )
        ...
        if "generation" in intents:
            ...
            if not any(elem in content_lower for elem in gen_elements):
                issues.append(
                    "Generation prompts should specify style, tone, or format"
                )
        ...
        if "classification" in intents:
            ...
            if not any(elem in content_lower for elem in class_elements):
                issues.append(
                    "Classification prompts should define the categories or classes"
                )
    ...
    def get_semantic_analysis(self, content: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "has_constraints": any(
                ...
                for word in ["must", "should", "required", "constraint"]
            ),
        }
