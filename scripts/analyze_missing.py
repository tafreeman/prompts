...
def analyze_missing_sections(file_path):
    ...
    required = {
        ...
        "Example": any(
            x in sections_lower for x in ["example", "example usage", "usage"]
        ),
    }
...
def main():
    ...
    for section, count in sorted(
        section_stats.items(), key=lambda x: x[1], reverse=True
    ):
    ...
    print(
        f"3. Most commonly missing: {max(section_stats.items(), key=lambda x: x[1])[0]} ({max(section_stats.values())} files)"
    )
    print()
    # Recommendation
    ...
    print(
        "  • Files missing sections: Add ## Description, ## Prompt, ## Variables, ## Example"
    )
    ...
    print(
        "  • Registry.yaml provides discovery metadata, files provide usage instructions"
    )
