...
def eval_prompts(path: str, tier: int = 2, output: Optional[str] = None):
    ...
    try:
        ...
        # Run evaluation
        if target.is_file():
            result = evaluate(str(target), tier=tier)
            results = {"results": [result], "prompts_evaluated": 1}
        else:
            results = evaluate_directory(str(target), tier=tier)
        ...
        print("   Make sure prompteval is available in tools/prompteval/")
    except Exception as e:
        print(f"❌ Error: {e}")
        ...
        for prompt_path in prompts:
            ...
            try:
                if provider == "local":
                    ...
                    _ = lm.generate(f"Evaluate this prompt (1-10): {content[:500]}", max_tokens=200)
                    print(" ✅")
                    results["processed"] += 1
                else:
