...
- **Python 3.11+** (the web app runs on Python 3.11; match it locally).
...
10. **Run linting scripts**:
    ```pwsh
    cd src
    python validate_prompts.py
    python load_prompts.py
    ```
    `validate_prompts.py` ensures the file is parsable; `load_prompts.py` refreshes `prompt_library.db` so the new prompt shows up in the web UI.
...
- **Forgetting governance metadata**: Security or compliance scenarios must declare `risk_level` and `approval_required`. Otherwise the analytics dashboard can’t surface the right controls.
- **Files not parsed**: A stray blank line before the opening `---` or inconsistent indentation can break `parse_markdown_prompt`. Keep the frontmatter contiguous and valid YAML.
...
| --- | --- | --- |
| `validate_prompts.py` reports parse failure | Missing `---` frontmatter or invalid YAML | Check for stray tabs, unclosed quotes, and ensure metadata keys exist. |
| Prompt doesn’t show up in the web app after running `load_prompts.py` | Prompt title duplicates an existing entry or `version` not higher | Use a unique title or update the metadata version; re-run `load_prompts.py` to refresh the database. |
...
- Browse `docs/getting-started.md` for guidance on using the web interface.
- Review `docs/best-practices.md` to align your prompt with the project’s tone and naming conventions.
- Inspect `src/README.md` for deployment, analytics, and verification contexts.
