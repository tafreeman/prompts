...
### 3.4 Dependency Resolution Model (DAG Execution)

**Rationale:** Sequential and Parallel patterns have implicit sync barriers. DAG execution achieves maximum parallelism by dynamically scheduling steps as their dependencies complete.

| Pattern | Sync Behavior | When to Use |
|---------|---------------|-------------|
| Sequential | Step N waits for N-1 | Strict order required |
| Parallel (Group) | Barrier after group completes | Known independent steps |
| **Graph (DAG)** | No artificial barriers | Complex dependency graphs |

**DAG Compilation from `depends_on`:**

The `StepDefinition.depends_on` field declares explicit dependencies. At runtime, the DAGExecutor:

1. **Validates** the graph (cycle detection via DFS with coloring)
2. **Computes** in-degree for each step (count of unmet dependencies)
3. **Schedules** steps with `in_degree == 0` immediately
4. **Decrements** dependents' in-degree when a step completes
5. **Repeats** until all steps complete or failure

```python
# DAG vs Pipeline execution
# Pipeline with parallel group:
#   [step1] -> [step2, step3] -> [step4]
#   Implicit barrier: step4 waits for BOTH step2 AND step3
#   Even if step2 finishes early, step4 doesn't start

# DAG execution:
#   step1 -> step2 -> step4
#   step1 -> step3 -> step4
#   step4 starts IMMEDIATELY when both step2 and step3 complete
#   No artificial waiting

dag = DAG("my-workflow")
dag.add(step1)                              # No dependencies
dag.add(step2.with_dependency("step1"))     # Depends on step1
dag.add(step3.with_dependency("step1"))     # Depends on step1
dag.add(step4.with_dependency("step2", "step3"))  # Depends on both
dag.validate()  # Raises CycleDetectedError if circular

result = await dag_executor.execute(dag, ctx, max_concurrency=10)
```
...
- **PENDING → SKIPPED**: `when=False` or `unless=True` condition
- **READY → RUNNING**: Executor schedules step (semaphore acquired)
- **RUNNING → SUCCESS/FAILED**: Step completes or errors with no retries left
- **RUNNING → RETRYING**: Error occurred but retries available

### 3.5 Strict Layout Enforcement

**Issue Identified:** The current implementation has loose files in the package root (`src/agentic_v2/`) that should be in their designated submodules:

| Current Location | Correct Location | Action |
|------------------|------------------|--------|
| `src/agentic_v2/agentmessage.py` | `src/agentic_v2/contracts/messages.py` | Move or delete if duplicate |
| `src/agentic_v2/basetool.py` | `src/agentic_v2/tools/base.py` | Move or delete if duplicate |
| `src/agentic_v2/agenticworkflowsv2config.py` | `src/agentic_v2/config/` | Move to config submodule |

**Enforcement Rule:** No Python modules should exist directly in `src/agentic_v2/` except:
- `__init__.py` (package exports)
- `py.typed` (PEP 561 marker)

All other code must reside in appropriate submodules:
- `contracts/` - Data models, schemas, validation
- `tools/` - Tool implementations and registry
- `agents/` - Agent implementations and registry
- `engine/` - Execution engine, context, patterns
- `models/` - LLM routing and client wrappers
- `workflows/` - Workflow definitions and registry
- `cli/` - Command-line interface
- `config/` - Configuration loaders and defaults

### 3.6 State Management & Checkpointing
