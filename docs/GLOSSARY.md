...
**Chain-of-Thought (CoT)** -- A prompting technique where the model reasons step-by-step before answering. All 24 agent personas include a `## Reasoning Protocol` section that scaffolds structured thinking. CoT improves accuracy on complex tasks by making the reasoning process explicit and auditable.
See `agentic_v2/prompts/*.md`.

**Chain-of-Verification (CoVe)** -- A pattern where claims generated by the model are independently verified after generation. In the `deep_research` workflow, dedicated `cove_verify` steps re-check factual claims against source material, flagging unsupported assertions. Each verification step has access to web search and HTTP tools for independent source confirmation.
See `agentic_v2/workflows/definitions/deep_research.yaml`.

**Confidence Gating** -- Conditional execution based on multi-dimensional confidence interval scores. In the `deep_research` workflow, each research round produces a confidence index (CI); iteration continues until `CI >= min_ci` (default 0.80) or `max_rounds` is reached. The `coverage_audit` step computes the CI from coverage score, source quality score, and recency metrics.
See `agentic_v2/workflows/definitions/deep_research.yaml`.

**ReAct (Reason + Act)** -- An agentic pattern where the agent reasons about the next action, executes it via a tool, observes the result, and repeats. In the codebase, ReAct manifests in retrieval steps where the researcher agent decides which searches to perform, executes them via `web_search` / `http_get` tools, and incorporates results into the next reasoning cycle.
See `agentic_v2/workflows/definitions/deep_research.yaml`, `agentic_v2/agents/base.py`.

**Tree-of-Thought (ToT)** -- A reasoning pattern that generates multiple reasoning paths (hypotheses) and evaluates them to select the most promising. In the `deep_research` workflow, `hypothesis_tree_tot` steps generate a tree of candidate hypotheses for each research round, which are then independently investigated and scored before selecting the strongest paths.
See `agentic_v2/workflows/definitions/deep_research.yaml`.
