...
function StatCardsLayout({ topic, onBack }) {
  const T = useTheme();
  const C = useChrome();
    ...
    <div style={{ position:"relative",minHeight:"100dvh",background:T.bg,overflowX:"hidden",overflowY:viewport.overlayScroll }}>
      ...
      <div style={{ position:"relative",zIndex:2,maxWidth:900,margin:"0 auto",padding:`${viewport.pagePaddingTop + 8}px ${viewport.pagePaddingX}px ${viewport.pagePaddingBottom}px`,opacity:e?1:0,transform:e?"translateY(0)":"translateY(30px)",transition:"all 0.8s cubic-bezier(0.22,1,0.36,1)" }}>
        ...
        {topic.cards.map((c,i)=>(<div key={i} style={{ background:T.bgCard,borderRadius:C.innerRadius,padding:viewport.isPhone?"20px 18px":"28px 32px",marginBottom:viewport.cardGap,display:"flex",flexDirection:viewport.isPhone?"column":"row",alignItems:viewport.isPhone?"stretch":"flex-start",gap:viewport.isPhone?14:24,borderLeft:`${C.accentBarHeight+1}px solid ${topic.color}`,opacity:e?1:0,transform:e?"translateY(0)":"translateY(20px)",transition:`all 0.6s ${0.3+i*0.15}s cubic-bezier(0.22,1,0.36,1)` }}>
          ...
          <div><h3 style={{ fontFamily:T.fontDisplay,fontSize:viewport.isPhone?16:18,fontWeight:C.headingWeight,color:topic.colorLight,margin:"0 0 8px" }}>{c.title}</h3><p style={{ fontSize:viewport.isPhone?13:14,color:T.textMuted,lineHeight:1.6,margin:0 }}>{c.body}</p></div>
        </div>))}
...
function ManifestStatCardsLayout({ topic, onBack }) {
  const T = useTheme();
  const C = useChrome();
  const viewport = usePresentationViewport();
      ...
      <div style={{ position: "relative", zIndex: 2, maxWidth: 1260, margin: "0 auto", padding: `${viewport.pagePaddingTop}px ${viewport.pagePaddingX}px ${viewport.pagePaddingBottom}px` }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: viewport.isCompact ? "1fr" : "1.15fr 0.85fr", gap: viewport.cardGap, marginBottom: 18 }}>
          <div style={{ opacity: entered ? 1 : 0, transform: entered ? "translateY(0)" : "translateY(22px)", transition: "all 0.6s ease" }}>
            ...
            <h1 style={{ fontFamily: T.fontDisplay, fontSize: viewport.titleSize, color: T.text, margin: "0 0 10px", lineHeight: 1.06 }}>{topic.heroTitle || topic.title}</h1>
            ...
            {topic.thesis && <p style={{ fontSize: viewport.bodySize, color: T.textMuted, lineHeight: 1.7, margin: 0 }}>{topic.thesis}</p>}
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: viewport.isPhone ? "1fr" : viewport.isCompact ? "1fr 1fr" : "repeat(3, minmax(0, 1fr))", gap: 14, marginBottom: 18 }}>
          {topic.cards.map((card, index) => (
            <div key={`${card.title}-${index}`} style={{ background: T.bgCard, borderRadius: C.cardRadius, padding: viewport.isPhone ? "16px 16px" : "18px 20px", borderTop: `${C.accentBarHeight}px solid ${topic.color}`, opacity: entered ? 1 : 0, transform: entered ? "translateY(0)" : "translateY(18px)", transition: `all 0.45s ${0.12 + index * 0.08}s ease` }}>
              ...
              <h3 style={{ fontFamily: T.fontDisplay, fontSize: viewport.isPhone ? 18 : 21, color: T.text, margin: "0 0 8px", lineHeight: 1.15 }}>{card.title}</h3>
        ...
        {topic.results?.length > 0 && (
          <div style={{ display: "grid", gridTemplateColumns: viewport.isPhone ? "1fr 1fr" : `repeat(${Math.min(topic.results.length, 4)}, minmax(0, 1fr))`, gap: 12, marginBottom: 18 }}>
            {topic.results.map((result, index) => (
              <div key={`${result.label}-${index}`} style={{ background: `linear-gradient(180deg, ${T.bgCard}, ${T.bgDeep})`, borderRadius: C.innerRadius, padding: viewport.isPhone ? "14px 14px" : "16px 18px", border: `${C.cardBorderWidth}px solid ${topic.color}18` }}>
...
ManifestStatCardsLayout.propTypes = {
  topic: PropTypes.object.isRequired,
  onBack: PropTypes.func.isRequired,
};
