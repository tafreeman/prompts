...
function BeforeAfterLayout({ topic, onBack }) {
  const T = useTheme();
  const C = useChrome();
          ...
          {topic.cards.map((c,i)=>{const v=i<vc,fl=i%2===0;const hiddenTransform=`translateX(${fl ? "-60px" : "60px"}) scale(0.92)`;return(
            <div key={i} style={{ background:T.bgCard,borderRadius:C.innerRadius,padding:viewport.isPhone?"18px 18px":"24px 28px",borderTop:`${C.accentBarHeight}px solid ${topic.color}`,position:"relative",overflow:"hidden",opacity:v?1:0,transform:v?"translateX(0) scale(1)":hiddenTransform,transition:"all 0.45s cubic-bezier(0.34,1.56,0.64,1)" }}>
              ...
              <div style={{ position:"relative",zIndex:1 }}>
                ...
                <div style={{ marginBottom:12 }}><div style={{ fontSize:10,textTransform:"uppercase",letterSpacing:1.2,fontWeight:700,color:T.danger,marginBottom:4 }}>Challenge</div><p style={{ fontSize:viewport.isPhone?12.5:13,color:T.textDim,lineHeight:1.5,margin:0 }}>{c.challenge}</p></div>
                <div><div style={{ fontSize:10,textTransform:"uppercase",letterSpacing:1.2,fontWeight:700,color:T.success,marginBottom:4 }}>Solution</div><p style={{ fontSize:viewport.isPhone?12.5:13,color:T.textMuted,lineHeight:1.5,margin:0 }}>{c.fix}</p></div>
              </div>
            </div>);})}
        </div>
