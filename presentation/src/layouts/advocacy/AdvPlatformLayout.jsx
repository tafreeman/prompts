...
export function AdvPlatformLayout({ topic, onBack }) {
  const T = useTheme();
  const C = useChrome();
  ...
  const heroPoints = topic.heroPoints || [];
  const focusPanels = topic.focusPanels || [];
  const capabilities = topic.capabilities || [];
  const lanes = topic.lanes || [];

  const toggleLane = (idx) => {
