...
if __name__ == "__main__":
    ...
    parser.add_argument('--no-cache', action='store_true', help='Force fresh fetch from source')
    args = parser.parse_args()
    
    use_cache = not args.no_cache
    print(f"Starting dataset fetch... (Cache: {'Enabled' if use_cache else 'Disabled'})")
    
    # Fetch HumanEval
    fetch_and_save("humaneval", map_humaneval, "humaneval.json", limit=None, use_cache=use_cache)
    ...
    fetch_and_save("swe-bench-lite", map_swebench, "swebench.json", limit=None, use_cache=use_cache)
