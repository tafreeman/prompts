...
Tests the MCP base classes, filesystem client, GitHub client, and registry.
"""
...
class TestFilesystemMCPClient:
    ...
    async def test_read_write_file(self, fs_client, temp_dir):
        ...
        write_response = await fs_client.invoke_tool("write_file", {
            "path": test_path,
            "content": "Hello, World!",
        })
        ...
        # Read it back
        read_response = await fs_client.invoke_tool("read_file", {
            "path": test_path,
        })
    ...
    async def test_list_directory(self, fs_client, temp_dir):
        ...
        response = await fs_client.invoke_tool("list_directory", {
            "path": temp_dir,
        })
    ...
    async def test_create_directory(self, fs_client, temp_dir):
        ...
        response = await fs_client.invoke_tool("create_directory", {
            "path": new_dir,
        })
    ...
    async def test_edit_file(self, fs_client, temp_dir):
        ...
        await fs_client.invoke_tool("write_file", {
            "path": test_path,
            "content": "Hello World",
        })
        
        # Edit it
        response = await fs_client.invoke_tool("edit_file", {
            "path": test_path,
    ...
    async def test_search_files(self, fs_client, temp_dir):
        ...
        response = await fs_client.invoke_tool("search_files", {
            "path": temp_dir,
            "pattern": "*.py",
        })
    ...
    async def test_get_file_info(self, fs_client, temp_dir):
        ...
        response = await fs_client.invoke_tool("get_file_info", {
            "path": test_path,
        })
    ...
    async def test_path_validation(self, fs_client, temp_dir):
        ...
        response = await fs_client.invoke_tool("read_file", {
            "path": "/etc/passwd",  # Outside allowed directory
        })
        
        assert response.success is False
        assert "outside allowed directories" in response.error.lower() or "not found" in response.error.lower()
    ...
    async def test_directory_tree(self, fs_client, temp_dir):
        ...
        response = await fs_client.invoke_tool("directory_tree", {
            "path": temp_dir,
        })
...
class TestMemoryMCPClient:
    ...
    async def test_upsert_get_list_search(self, tmp_path: Path):
        ...
        # Search
        searched = await client.invoke_tool("search", {"query": "workflows", "limit": 10})
...
class TestMCPRegistry:
    ...
    async def test_invoke_tool(self, registry):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            response = await registry.invoke_tool("fs", "read_file", {"path": test_file})
...
class TestMCPRegistryGlobal:
    ...
    def test_get_mcp_registry(self):
        ...
        from multiagent_workflows.mcp.registry import get_mcp_registry, _mcp_registry
