...
class TestDatasetIntegration(unittest.TestCase):
    """
    Test that the dataset tool correctly links to authoritative sources
    and can fetch fresh data.
    """

    def test_humaneval_source_definition(self):
        """Verify HumanEval is configured to pull from OpenAI's HuggingFace repo."""
    ...
    def test_swebench_lite_source_definition(self):
        """Verify SWE-bench Lite is configured to pull from Princeton-NLP's repo."""
    ...
    def test_fresh_fetch_invocation(self, mock_check, mock_load):
        """
        Verify that attempting to load with use_cache=False triggers
        the direct loader for the specific source.
        """
    ...
    def test_github_source_link(self, mock_urlopen):
        """
        Verify that GitHub-sourced benchmarks link to the correct API endpoint.
        """
        # Mock response setup
        mock_list_response = MagicMock()
        mock_list_response.read.return_value = b'[{"name": "task_1.json", "download_url": "http://github.com/d/t1.json"}]'
