...
WORKFLOWS_CONFIG_DIR = (
    Path(__file__).resolve().parents[1] / "config" / "agentic_planning"
)
...
class TestAgentConfigurations:
    ...
    def test_all_agents_have_required_fields(
        self, workflow_config_loader, workflow_name
    ):
        ...
        for agent in config["agents"]:
            for field in required_fields:
                assert (
                    field in agent
                ), f"Agent {agent.get('id', 'unknown')} missing {field}"
    ...
    def test_all_agents_have_compatible_models(
        self, workflow_config_loader, workflow_name
    ):
        ...
        for agent in config["agents"]:
            assert (
                "compatible_models" in agent
            ), f"Agent {agent['id']} has no fallback models"
            assert (
                len(agent["compatible_models"]) >= 1
            ), f"Agent {agent['id']} needs at least 1 fallback"
    ...
    def test_all_agents_have_phase_assignment(
        self, workflow_config_loader, workflow_name
    ):
...
class TestModelTierAssignments:
    ...
    def test_judge_agents_use_premium_models(
        self, workflow_config_loader, workflow_name
    ):
    ...
    def test_intake_agents_use_efficient_models(
        self, workflow_config_loader, workflow_name
    ):
        ...
        for agent in config["agents"]:
            ...
            if is_intake:
                ...
                assert (
                    tier in self.TIER_WEIGHTS or tier == ""
...
class TestEvaluatorIntegration:
    ...
    def test_workflow_config_compatible_with_evaluator_rubric(
        self, workflow_config_loader
    ):
        ...
        for our_workflow, rubric_name in mapping.items():
            ...
            if rubric_name:
                ...
                assert (
                    len(phases) >= 3
                ), f"Workflow {our_workflow} needs more phases for evaluation"
