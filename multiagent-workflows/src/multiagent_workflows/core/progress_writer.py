...
Writes workflow execution state to a JSON file that can be
read by the web-based dashboard for live visualization.
"""
...
class DashboardProgressWriter:
    ...
    def _save_runs(self):
        ...
        with open(self.runs_file, "w") as f:
            json.dump({"runs": self.runs, "updated_at": datetime.now().isoformat()}, f, indent=2)
    ...
    def complete_run(self, run_id: str, outputs: Dict[str, Any], error: Optional[str] = None):
