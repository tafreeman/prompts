...
class WorkflowEvaluator:
    ...
    def _load_rubrics(self) -> Dict[str, Any]:
        ...
        rubrics_path = (
            Path(__file__).parent.parent.parent.parent / "config" / "rubrics.yaml"
        )
        if not rubrics_path.exists():
            # Fallback to config directory relative to current file
            rubrics_path = (
                Path(__file__).parent.parent.parent / "config" / "rubrics.yaml"
            )
    ...
    def has_golden(self, workflow_name: str, inputs: Dict[str, Any]) -> bool:
        ...
        golden_dir = Path(
            self.config.get("paths", {}).get("golden", "./evaluation/golden")
        )
    ...
    def _calculate_grade(self, score: float) -> str:
        ...
        grade_scale = self.rubrics.get("scoring", {}).get(
            "grade_scale",
            {
                "A": [90, 100],
                "B": [80, 89],
    ...
    def _generate_feedback(
        ...
        if sorted_cats:
            ...
            if len(sorted_cats) > 1:
                ...
                feedback += (
                    f"Area for improvement: {bottom[0]} ({bottom[1]['score']:.0f}%)."
                )
    ...
    async def generate_report(
        ...
        for r in results:
            ...
            lines.append(
                f"| {r.workflow_name} | {r.total_score:.1f}% | {r.grade} | {status} |"
            )
