...
class AgentBase(ABC):
    ...
    async def _process(
        ...
        """Fallback core processing logic for agents without a custom
        implementation.

        Calls the model and tries to parse JSON from the response,
        returning structured output or a raw_response field.
        """
        prompt = (
            task.get("prompt")
