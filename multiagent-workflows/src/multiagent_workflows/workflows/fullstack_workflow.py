...
class FullStackWorkflow(BaseWorkflow):
    ...
    Takes business requirements and produces a complete deployable
    application with backend, frontend, tests, and documentation.
    """
        ...
        return [
            ...
            WorkflowStep(
                ...
                outputs=[
                    "backend_code",
                    "frontend_code",
                ],  # Overwrites previous code artifacts
            ),
            WorkflowStep(
    ...
    def step_dependencies(self):
        """Returns a dictionary mapping step names to their dependencies for
        parallel execution."""
        return {
    ...
    async def _create_agent(self, step: WorkflowStep):
        ...
        agent_details: Dict[str, Dict[str, Any]] = {
            "requirements_analyzer": {
                "class": SimpleAgent,
                "pref": "reasoning",
                "prompt": "You are a senior business analyst. Analyze the provided requirements thoroughly. Output a JSON object with keys: user_stories, data_entities.",
            },
            "system_architect": {
                "class": ArchitectAgent,
                "pref": "reasoning",
                "prompt": "You are a senior system architect. Create a complete system architecture including: ARCHITECTURE DIAGRAM (in MermaidJS), SERVICE DECOMPOSITION, and TECH STACK. Output a JSON object with keys: architecture, tech_stack.",
            },
            "database_architect": {
                "class": SimpleAgent,
                "pref": "reasoning",
                "prompt": "You are a database architect. Design the database schema and migrations. Output a JSON object with keys: schema, migrations.",
            },
            "api_designer": {
                "class": SimpleAgent,
                "pref": "reasoning",
                "prompt": "You are an API design expert. Design the API contracts and provide a complete OpenAPI 3.0 specification. Output a JSON object with keys: api_spec, endpoints.",
            },
            "backend_generator": {
                "class": CoderAgent,
                "pref": "code_gen",
                "prompt": "You are a senior backend developer. Generate backend code following clean architecture and secure coding practices. Output a JSON object with keys: backend_code, backend_files.",
            },
            "frontend_generator": {
                "class": CoderAgent,
                "pref": "code_gen",
                "prompt": "You are a senior frontend developer. Generate frontend code following best practices for component structure and state management. Output a JSON object with keys: frontend_code, frontend_files.",
            },
            "reviewer_agent": {
                "class": ReviewerAgent,
                "pref": "code_review",
                "prompt": "You are a senior security engineer and code reviewer. Review for SECURITY, PERFORMANCE, and MAINTAINABILITY. Output a JSON object with keys: review_results, issues.",
            },
            "code_refiner": {
                "class": CoderAgent,
                "pref": "code_gen",
                "prompt": "You are a senior developer. Refine the provided code based on the review feedback to fix all identified issues. If there are no issues, return the original code. Output a JSON object with keys: backend_code, frontend_code.",
            },
            "test_generator": {
                "class": TestAgent,
                "pref": "code_gen",
                "prompt": "You are a QA engineer. Generate comprehensive tests including Unit, Integration, and E2E tests. Output a JSON object with keys: tests, test_files.",
            },
            "documentation_writer": {
                "class": SimpleAgent,
                "pref": "documentation",
                "prompt": "You are a technical writer. Generate a README and API documentation. Output a JSON object with keys: readme, api_docs.",
            },
        }
    ...
    def to_langgraph(self, output_path: Optional[str] = None) -> Any:
        ...
        try:
            ...
            if self.logger:
                self.logger.warning(
                    "LangGraph not installed or dependencies missing. Cannot export."
                )
            return None
        ...
        for step in steps:
            ...
            workflow.add_node(
                step.name, lambda state: {step.outputs[0]: f"Generated by {step.agent}"}
            )
        ...
        for step in steps:
            if (
                step.name not in all_deps and step.name != "code_refinement"
            ):  # logic exception for cycle
            ...
            try:
                ...
                if self.logger:
                    ...
                    logging.getLogger(__name__).info(
                        f"LangGraph exported to {output_path}"
                    )
            except Exception as e:
                import logging

                logging.getLogger(__name__).error(
                    f"Failed to save LangGraph export: {e}"
                )
