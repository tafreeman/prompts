...
from .parse import parse_frontmatter, extract_sections, extract_body
from .config import (
...
def validate(path: str | Path) -> ValidationResult:
    ...
    if not fm:
        ...
        if not fm.get("description") and not fm.get("intro"):
            result.add_warning(
                "Missing description or intro field",
                field="description"
            )
...
def _check_common_issues(content: str, result: ValidationResult) -> None:
    ...
    if prompt_match:
        ...
        block_match = re.search(r'##\s+Prompt\s*\n+```[^\n]*\n(.*?)```', 
    ...
    vars_section = re.search(r'##\s+Variables?\s*\n(.*?)(?=##|\Z)', 
                             content, re.IGNORECASE | re.DOTALL)
    ...
    for i, line in enumerate(lines, 1):
        if len(line) > 200 and not line.startswith('|'):  # Skip table rows
            result.add_info(
                f"Line {i} is very long ({len(line)} chars)",
                line=i
            )
