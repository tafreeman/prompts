...
def evaluate_batch(
    ...
    for i, p in enumerate(paths):
        ...
        result = _evaluate_single(
            Path(p), model, method, tier, threshold, verbose=False
        )
...
def _evaluate_single(
    ...
    return _evaluate_with_llm(
        path, content, model, method, tier, threshold, start_time, verbose
    )


def _evaluate_structural(
    path: Path, content: str, threshold: float, start_time: float
) -> EvalResult:
...
def _evaluate_with_llm(
    ...
    if cached_response:
        ...
        return _parse_llm_response(
            cached_response, path, model, method, tier, threshold, start_time
        )
    ...
    return _parse_llm_response(
        response, path, model, method, tier, threshold, start_time
    )
