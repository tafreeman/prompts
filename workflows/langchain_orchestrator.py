...
def stub_criteria_validator(manifest: dict):
    ...
    return {'criteria_valid': True, 'effective_criteria': effective, 'evidence': evidence}
...
def stub_scoring(prompt_path: str, effective_criteria: dict):
    ...
    per = {k: round(max(0.0, min(10.0, random.gauss(7.0, 1.0))), 2) for k in effective_criteria.keys() or ['clarity','effectiveness','specificity']}
...
def stub_implementer(per_criterion: dict):
    ...
    updated_prompt = f"[UPDATED PROMPT - added output format]\n" + "\n".join([f"# Criterion {k}: target {v}" for k, v in per_criterion.items()])
...
def stub_validator(updated_prompt: str):
    ...
    observed = {'passed': passed, 'issues': [] if passed else ['missing explicit JSON schema']}
    return observed


def orchestrate(manifest: dict, iteration_plan: dict, workflow_map: dict, prompt_path: str, run_dir: Path, run_id: str, timestamp: str):
    ...
    summary = {
        ...
        'per_criterion_mean': round(statistics.mean(all_scores), 2) if all_scores else None,
...
def main():
    ...
    iteration_plan = load_yaml(iteration_plan_path) if iteration_plan_path.exists() else {}
    workflow_map = load_yaml(workflow_path) if workflow_path.exists() else {}

    run_id = manifest.get('run_id') or f"dryrun_{datetime.utcnow().strftime('%Y%m%dT%H%M%SZ')}"
    timestamp = datetime.utcnow().strftime('%Y%m%dT%H%M%SZ')
    run_dir = Path(args.output_dir) / run_id
    run_dir.mkdir(parents=True, exist_ok=True)

    out_path = orchestrate(manifest, iteration_plan, workflow_map, prompt_path, run_dir, run_id, timestamp)
