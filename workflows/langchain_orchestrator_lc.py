"""Orchestrator that uses LangChain chains when available, otherwise falls back
to stubs.
...
def main():
    ...
    p.add_argument(
        "--max-iterations", type=int, default=None, help="Override max iterations"
    )
    args = p.parse_args()

    manifest = load_yaml(Path(args.manifest)) if Path(args.manifest).exists() else {}
    iteration_plan = (
        load_yaml(Path(args.iteration_plan))
        if Path(args.iteration_plan).exists()
        else {}
    )
    workflow_map = (
        load_yaml(Path(args.workflow)) if Path(args.workflow).exists() else {}
    )

    run_id = (
        manifest.get("run_id")
        or f"langchain_{datetime.utcnow().strftime('%Y%m%dT%H%M%SZ')}"
    )
    ...
    try:
        ...
        if not model_name:
            raise RuntimeError(
                "No model_version in manifest and no discovered model found."
            )
    ...
    for iteration in range(1, max_iters + 1):
        ...
        effective_criteria = cv_res.get(
            "effective_criteria", manifest.get("grading_criteria", {})
        )
        # Scoring (can be parallelized)
        scoring_res = scoring_chain.run(
            prompt_text=prompt_text, criteria_json=json.dumps(effective_criteria)
        )
        ...
        if weighted_score >= quality_threshold:
            print(
                f"Terminating: weighted_score {weighted_score} >= quality_threshold {quality_threshold}"
            )
            break
