...
Engineering rigor — typed, tested, immutable-first, deterministic where possible.
Research grounding — claims cite peer-reviewed papers, official vendor docs, or reputable engineering blogs with valid URLs published within the last 6 months.
Evaluation readiness — every workflow, prompt, and tool must be measurable via the eval framework.
...
Intake/Scoping — goal, constraints, recency window
Planner — Tree of Thoughts (ToT) hypothesis branching
Retrieval — ReAct loops with search tools
Analysis — Domain-specialist personas (AI Engineer, SWE Analyst)
Verification — Chain-of-Verification (CoVe): draft → verify → revise
Auditing — Coverage + confidence scoring (CI >= 0.80)
Supervisor — re-runs gaps until thresholds met
Synthesis — Top-tier model writes final report
RAG Packaging — Chunked evidence + claim graph
...
Critical rules — numbered, unambiguous constraints
Output format — structured schema
Rework mode — handle review_report + suggested_fixes inputs
...
DimensionWeightDescriptionCompleteness0.25Addresses all requirementsCorrectness0.25Technically correct, best practicesQuality0.20Well-structured, maintainableSpecificity0.15Actionable details, not genericAlignment0.15Matches gold-standard expectations
6.3 Eval-Driven Development

Before building — define rubric criteria for "good"
During building — trace grading data for reproducible datasets
After building — run eval suite; regression gates must pass
Iterate — use results to refine prompts, tools, workflows
...
7. LLM Model Routing
TierUse CasePrimary Models0Deterministic (no LLM)Templates, regex, math1Fast/cheap — extractiongemini-2.0-flash-lite, gpt-4o-mini2Balanced — code reviewgemini-2.0-flash, gpt-4o, claude-sonnet-43Strong reasoning — architecturegemini-2.5-flash, claude-sonnet-44Top-tier — planning(availability-gated from tier 3)5Premium — deep research(best available)
...
Identify existing files to modify vs new files to create
Prefer editing existing patterns over new abstractions
Check if eval rubric criteria need updating
...
Never produce TODOs in generated code. All files must be complete and runnable.
...
Never use high-risk tools in research/eval workflows.
Never produce research claims without citations and valid URLs.
