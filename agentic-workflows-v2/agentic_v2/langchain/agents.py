...
# Tier → model name mapping.  Override via env vars or pass explicit models.
DEFAULT_TIER_MODELS: dict[int, str] = {
    0: "",                         # Tier 0 = deterministic, no LLM
    1: "gpt-4o-mini",
    2: "gpt-4o",
    3: "gpt-4o",
    4: "gpt-4o",
...
def _get_chat_model(tier: int, model_override: str | None = None) -> BaseChatModel:
    """Instantiate a ChatModel for the given tier.

    Uses ``langchain_openai.ChatOpenAI`` by default.  Override by
    setting ``AGENTIC_MODEL_TIER_{N}`` environment variables or by
    passing ``model_override``.
    """
    model_name = (
        model_override
        or os.environ.get(f"AGENTIC_MODEL_TIER_{tier}")
        or DEFAULT_TIER_MODELS.get(tier, "gpt-4o")
    )

    # Dynamic import so the openai extra isn't required at import time
    try:
        from langchain_openai import ChatOpenAI

        return ChatOpenAI(model=model_name, temperature=0)
    except ImportError:
        # Fallback: let the user provide their own ChatModel
        raise ImportError(
            "langchain-openai is required.  "
            "Install with: pip install 'agentic-workflows-v2[langchain]'"
        )
...
def create_agent(
    ...
    *,
    ...
    # Resolve model
    model = _get_chat_model(tier, model_override)
