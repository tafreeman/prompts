...
(agentic_v2.langchain) and is also triggered transitively when the adapter
registration path (agentic_v2.adapters.langchain) is imported.  The adapter
test requires langgraph; the module-level test does not because the warning
fires before any langgraph-dependent code.
"""
...
def test_langchain_adapter_emits_deprecation_warning() -> None:
    ...
    import agentic_v2.adapters.langchain  # noqa: F401
    ...
    assert any("ADR-013" in msg for msg in messages), (
        f"ADR-013 not found in warning messages: {messages}"
    )
    assert any("LangGraph" in msg or "langchain" in msg for msg in messages), (
...
def test_langchain_module_emits_deprecation_warning() -> None:
    ...
    pytest.importorskip("langgraph")
    import agentic_v2.langchain  # noqa: F401
...
def test_langchain_module_warns_without_langgraph() -> None:
    ...
    common user scenario (langgraph not installed) by reloading the module
    directly — no langgraph dependency required.
    """
    import agentic_v2.langchain  # noqa: F401
