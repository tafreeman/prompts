...
try:
    ...
    raise ImportError(
        "Could not import tools.llm.llm_client. Run this from the repo root."
    ) from e
...
def main():
    ...
    demo_prompt = "Summarize the following in one sentence: LangChain integration demo for the prompts repo."
    ...
    try:
        ...
        print(
            "Imported LLMChain from langchain.chains and PromptTemplate from langchain.prompts"
        )
    except Exception:
        try:
            ...
            print(
                "Imported LLMChain from langchain and PromptTemplate from langchain.schema"
            )
        except Exception as e:
    ...
    if (
        adapter is not None
        and "LLMChain" in globals()
        and globals()["LLMChain"] is not None
    ):
        try:
